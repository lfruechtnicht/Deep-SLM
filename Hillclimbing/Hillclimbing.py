...
class Hillclimbing():#todo ABC META
    ...
    def _get_elite(self):
        ...
        self.neighborhood.sort(key=lambda x: x.fitness, reverse=self.metric.greater_is_better)  # could do argmax or arg min
        
        """ TODO: -Lukas _get_elite """
        if self.neighborhood[0].fitness > self.elite.fitness:
            print("Could not decrease loss, best child = %.5f, parent = %.5f" % (self.neighborhood[0].fitness, self.elite.fitness))

...
class ErrorDeviationVariation(StoppingCriterion):
    ...
    def evaluate(self, algorithm):
        ...
        var_superior_solutions = [std(abs(superior_solution.predictions - algorithm.target_vector)) for
                                  superior_solution in superior_solutions]
...
class TrainingImprovementEffectiveness(StoppingCriterion):
    ...
    def evaluate(self, algorithm):
        ...
        superior_solutions = [solution for solution in algorithm.population if
                              is_better(solution.value, champion.value, algorithm.metric)]
        # if not superior offspring exist, determine parent stopping criterion
        if not superior_solutions:
            return super().evaluate(algorithm)
